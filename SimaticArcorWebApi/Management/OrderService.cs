...
namespace SimaticArcorWebApi.Management
{
    public class OrderService : IOrderService
    {
            ...
            foreach (ProductionRequestOperations o in prod.Operations)
            {
                ...
                List<IOOrderOperationParameterRequirements> oParamterReqList = new List<IOOrderOperationParameterRequirements>();
                    ...
                    foreach (ProductionRequestOperationParameters p in o.Parameters)
                    {
                        ...
                        else if (material.NId.StartsWith("31"))
                        {
                            flagRevision = true;
                        }
                        ...
                        else if (material.NId.StartsWith("37"))
                        {
                            flagRevision = true;
                        }
        ...
        public async Task<WorkOrderOperationParameterSpecification> UpdateCounterTrimCorte(string parameterId, CancellationToken ct)
        {
        ...
        public async Task ProcessNewOperationAsync(WorkOrderOperationRevisionRequest prod, CancellationToken ct)
        {
            ...
            if (!string.IsNullOrEmpty(prod.Id))
            {
                ...
                if (woOperations.Count > 0)
                {
                    ...
                    foreach (ProcessParameter itemParameter in prod.ProcessParameters)
                    {
                ...
                if (woOperations.Count > 0)
                {
                    ...
                    foreach (MaterialsQuantity itemMaterial in prod.materialsQuantity)
                    {
                        ...
                        if (materialRequirements.Count == 0)
                        {
                            ...
                            itemMaterialRequirement.Direction = "";
        ...
        #region [Obsolete] async UpdateOrder
        //private async Task UpdateOrder(ProductionRequest prod, Order currentOrder, WorkOrder wo, CancellationToken ct)
        //{
        //  if (currentOrder.PlannedStartTime == prod.StartTime && currentOrder.PlannedEndTime == prod.EndTime &&
        //      currentOrder.Quantity.QuantityValue == prod.Quantity)
        //  {
        //    return;
        //  }
        ...
        //  await SimaticService.UpdateOrderAsync(currentOrder.Id, currentOrder.Name, currentOrder.Description,
        //    currentOrder.ActualStartTime, currentOrder.ActualEndTime, currentOrder.EquipmentNId, currentOrder.MaterialNId,
        //    currentOrder.MaterialRevision, prod.StartTime, prod.EndTime, currentOrder.Status.StateMachineNId, prod.Quantity,
        //    currentOrder.Quantity.UoMNId, currentOrder.Type, ct);

        //  if(wo != null)
        //    await SimaticService.UpdateWorkOrderExtendedV2(wo.Id, prod.StartTime, prod.EndTime, prod.Quantity,wo.Quantity.UoMNId, ct);
        //}

        //private async Task UpdateWorkOrderUserFields(IList<WorkOrderUserField> userfields, ProductionRequest prod, CancellationToken ct)
        //{
        //  //var ufExportNbr = userfields.FirstOrDefault(t => t.NId == nameof(prod.ExportNbr));
        //  //if (ufExportNbr != null)
        //  //  await SimaticService.UpdateWorkOrderUserField(ufExportNbr.Id, prod.ExportNbr, ct);

        //  //var ufLine = userfields.FirstOrDefault(t => t.NId == nameof(prod.Line));
        //  //if (ufLine != null)
        //  //  await SimaticService.UpdateWorkOrderUserField(ufLine.Id, prod.Line, ct);

        //  //var ufShipmentOrder = userfields.FirstOrDefault(t => t.NId == nameof(prod.ShipmentOrder));
        //  //if (ufShipmentOrder != null)
        //  //  await SimaticService.UpdateWorkOrderUserField(ufShipmentOrder.Id, prod.ShipmentOrder, ct);

        //  //var ufType = userfields.FirstOrDefault(t => t.NId == "Type");
        //  //if (ufType != null)
        //  //  await SimaticService.UpdateWorkOrderUserField(ufType.Id, prod.FamilyID, ct);

        //  //var ufPlant = userfields.FirstOrDefault(t => t.NId == "Planta");
        //  //if (ufPlant != null)
        //  //  await SimaticService.UpdateWorkOrderUserField(ufPlant.Id, prod.PlantID, ct);

        //  // TODO: modificar estos 2 UserFields para mandar lo que se recibe por la interfaz
        //  //var ufRoadMapNId = userfields.FirstOrDefault(t => t.NId == "RoadMapNId");
        //  //if (ufRoadMapNId != null)
        //  //    await SimaticService.UpdateWorkOrderUserField(ufRoadMapNId.Id, prod.FinalMaterial, ct);

        //  //var ufRoadMapType = userfields.FirstOrDefault(t => t.NId == "RoadMapType");
        //  //if (ufRoadMapType != null)
        //  //    await SimaticService.UpdateWorkOrderUserField(ufRoadMapType.Id, "M", ct);
        //}

        //private async Task CreateOrUpdateUserField(string key, string value, string orderId, IList<OrderUserField> fields, CancellationToken ct)
        //{
        //  var item = fields.FirstOrDefault(i => i.NId == key);
        //  if (item == null)
        //  {
        //    //Add item
        //    await SimaticService.CreateOrderUserField(orderId, key, value, ct);
        //  }
        //  else
        //  {
        //    if (item.UserFieldValue != value)
        //    {
        //      //Update item
        //      await SimaticService.UpdateOrderUserField(item.Id, value, ct);
        //    }
        //  }
        //}
        #endregion
