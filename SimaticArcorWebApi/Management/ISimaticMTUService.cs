...
namespace SimaticArcorWebApi.Management
{
    public interface ISimaticMTUService
    {
        ...
        Task<dynamic> GetMTUPropertiesAsync(Guid id, CancellationToken token);
        ...
        Task CreateMaterialTrackingUnitProperties(Guid id, MTURequestMaterialLotProperty[] properties, CancellationToken token);
        ...
        Task SetMaterialTrackingUnitQuantity(Guid id, string uoMnId, double quantity, CancellationToken token);

        Task SetMaterialTrackingUnitStatus(Guid id, string verb, CancellationToken token);

        Task MoveMaterialTrackingUnitAsync(Guid id, double quantity, string equipment, CancellationToken token);

        Task MoveMaterialTrackingUnitToEquipmentAsync(Guid id, string equipment, CancellationToken token);

        Task UpdateMTUAsigned(string id, decimal quantity, CancellationToken token);

        Task UpdateMaterialTrackingUnitAsync(Guid id, string name, string description, string status, CancellationToken token);

        Task UpdateMaterialTrackingUnitProperties(Guid id, MTURequestMaterialLotProperty[] properties, CancellationToken token);
        ...
        Task<dynamic> GetPropertiesDefectos(Guid Id, CancellationToken ct);

        Task<dynamic> GetPrinter(string equipmentNId, CancellationToken token);

        Task<dynamic> GetLotePadreProp(Guid Id, CancellationToken token);

        Task<dynamic> GetUniqueId(Guid Id, CancellationToken token);
        #endregion
        ...
        Task<dynamic> GetPropertyWorkOrder(Guid Id, CancellationToken token);
        Task<dynamic> GetMaquinaDeCreacion(Guid Id, CancellationToken token);
